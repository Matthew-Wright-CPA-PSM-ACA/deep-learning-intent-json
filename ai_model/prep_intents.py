...
# - utilization allowed under the MIT license. 
...
if fileType == "json":
	...
	for intent in intents['intents']:
		try: 
			phrase.sentence = str(intent['tag'])
			phrase.prep_intent_sentence()
			intentVal = phrase.sentence

			if phrase.sent_length > MAX_LENGTH:
				MAX_LENGTH = phrase.sent_length

			#print(f'\nThe intent is: {intentVal} \n')

			for pattern in intent['patterns']:

				phrase.sentence = str(pattern)
				phrase.prep_intent_sentence()

				if phrase.sent_length > MAX_LENGTH:
					MAX_LENGTH = phrase.sent_length

				questions.append(phrase.sentence)
...
# This will list the current intents
if listIntentsQA == True:
	for x in answers:
		if x not in tempIntents:
			tempIntents.append(x)	
	
	print(f'The current intents are: {tempIntents}')
	
	sys_exit() #exit the program. Simple way.

#Simplified attempt here
#if listIntentsQA == True:
	#tempIntents = [x for x in answers if x not in tempIntents]
	#print(f'tempV2: {tempIntents}')
	
# // end of listing current intents
...
def evaluate():
	...
	input_sent = BEGIN_TOKEN + tokenizer.encode(phrase.sentence) + ENDING_TOKEN

	phrase.sentence = tf_expand_dims(input_sent, axis=0)

	output = tf_expand_dims(BEGIN_TOKEN, 0)
	
	for i in range(MAX_LENGTH):
		predictions = model(inputs=[phrase.sentence, output], training=False)
